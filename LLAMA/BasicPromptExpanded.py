...
def create_prompt(new_sentence, examples):
    prompt = "You are a fact checker detecting claims. " \
             ...
             "Classify the following sentences into one of the categories: Yes, No.\n "
...
def classify_sentence(prompt):
    ...
    output = model.generate(**inputs, max_length=inputs['input_ids'].shape[1] + 3, num_return_sequences=1, pad_token_id=tokenizer.eos_token_id)
...
with open("Results/resultsBasicLLAMA.txt", "w", encoding="utf-8") as file:
